...
var pmCollection = require('./TMF707-TestResult-v4.0.0.testkit.json');
...
Object.keys(config['payloads']).forEach(resource => {
    ...
    if (!valid.valid) {
        ...
        process.exit(0)
                    ...
                    if (resource == "TestCaseResult") {
                        pmCollection['item'][1]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
                    }
                
                    if (resource == "TestSuiteResult") {
                        pmCollection['item'][4]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
                    }
                
                    if (resource == "NonFunctionalTestResult") {
                        pmCollection['item'][7]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
                    }
...
function exportEnvironment(url) {
    ...
    envJson.values.forEach(element => {
        if (element.key == "Test_Result") {
            element.value = config['url'];
        }
    });
    console.log("Testing Resource: " + config['resourcesToTest'])
    if (config['resourcesToTest']){
        resToTest = {
            "key": "resourceToTest",
            "value": config['resourcesToTest'],
            "enabled": true
        }
        envJson.values.push(resToTest)
    }
